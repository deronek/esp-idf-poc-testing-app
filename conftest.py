...
from utils import get_idf_path, get_sw_variant, get_hw_variant, get_correct_dac_setting_commands, get_status_command, \
    get_variant_command, get_incorrect_commands

os.chdir(get_idf_path())

try:
    from idf_ci_utils import to_list
    from idf_unity_tester import CaseTester
except ImportError:
    sys.path.append(os.path.join('tools', 'ci'))
    from idf_ci_utils import to_list
    from idf_unity_tester import CaseTester
...
try:
    ...
    sys.path.append(os.path.join('tools', 'ci', 'python_packages'))
    import common_test_methods  # noqa: F401
...
ENV_MARKERS = {
    ...
    # MD: added
    'poc_app_functional': 'Functional testing for proof-of-concept app',
    'poc_app_fuzzing': 'Fuzzing testing for proof-of-concept app',
    'poc_app_performance': 'Performance testing for proof-of-concept app',
    'poc_app_smoke': 'Smoke testing for proof-of-concept app',
}
...
def test_case_name(request: FixtureRequest, target: str, config: str) -> str:
    return format_case_id(target, config, request.node.name)
...
@pytest.fixture(autouse=True)
def set_expect_timeout(dut):
    dut.pexpect_proc.timeout = 3


# Correct commands - numbers 1 through 4
@pytest.fixture(scope='module', params=get_correct_dac_setting_commands())
def correct_dac_setting_command(request) -> str:
    return request.param


@pytest.fixture(scope='module', params=get_status_command())
def status_command(request) -> str:
    return request.param


@pytest.fixture(scope='module', params=get_variant_command())
def variant_command(request) -> str:
    return request.param


@pytest.fixture(scope='module', params=get_incorrect_commands())
def incorrect_command(request) -> str:
    return request.param
...
def pytest_configure(config: Config) -> None:
    ...
    # Call SW_VARIANT and HW_VARIANT getters, to confirm the env vars are set properly
    get_sw_variant()
    get_hw_variant()
                ...
                if failed_sub_cases:
                    ...
                    if test_case_name:
                        self._failed_cases.append(
                            (
                                test_case_name, self._is_known_failure(test_case_name),
